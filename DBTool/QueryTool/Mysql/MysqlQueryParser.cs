namespace NFramework.DBTool.QueryTool.Mysql
{
    ...
    /// 
    /// </summary>
    public class MysqlQueryParser : QueryParser
    {
        public new List<MySqlParameter> ParamCollection
        {
            get;
            set;
        }
        ...
        {
            this.ParamCollection = new List<MySqlParameter>();
        }
        ...
        protected override string LikeParse(LikeCondition condition)
        {
            ...
            else
            {
                string paramName = "P" + Guid.NewGuid().ToString().ToLower().Replace("-", "");
                conditionStr.Append("@" + paramName);
                ParamCollection.Add(new MySqlParameter(paramName, "%" + condition.ParamValue.ToString() + "%"));
            }
        ...
        /// <param name="condition">需要解析的条件对象</param>
        /// <returns>返回解析后的条件字符串</returns>
        protected override string NotExistsParse(NotExistsCondition condition)
        {
            StringBuilder conditionStr = new StringBuilder(" ");
            string queryColName = condition.SearchColumn.CurrentSearcher.TableName + "." + condition.SearchColumn.ColumnName;
            ...
            conditionStr.Append(" NOT Exists ( ");
            conditionStr.Append(condition.SQL);
            conditionStr.Append(") ");
